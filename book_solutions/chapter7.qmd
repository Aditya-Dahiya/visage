...
outline_map <- rnaturalearth::ne_coastline(
  returnclass = "sf",
  scale = "large"
  ) |> 
  st_union() |> 
...
row_1_title <- "Without any CRS"
row_1_subtitle <- "Using London data interpreted as Geographic Projection, i.e. as Longitude and Latitude"
sf_use_s2(TRUE)
g1 <- base_plot +
  geom_sf(
  ...
  labs(
    title = "Buffer of 1 degree",
    subtitle = "Using S2 engine"
  )
...
g2 <- base_plot +
  ...
  labs(
    title = "Buffer of 1 degree (S2 disabled)",
    subtitle = "Using GEOS engine"
  )
...
g3 <- base_plot +
  ...
  labs(
    title = "Buffer of 1 degree",
    subtitle = "Using S2 engine with nQuadSegs = 2"
  )


row_2_title <- "London with EPSG:4326, Long-Lat, but distance in metres"
row_2_subtitle <- "Using London data in a Geographic CRS, i.e. World Geodetic System 1984 ensemble"
sf_use_s2(TRUE)
...
g4 <- base_plot +
  ...
  labs(
    title = "Buffer of 1 - interpreted as 1 metre",
    subtitle = "Using S2 engine"
  )
...
g5 <- base_plot +
  ...
  labs(
    title = "Buffer of 1 (without S2 engine)",
    subtitle = "Used GEOS. Interpreted as degrees; Again incorrect."
  )
...
g6 <- base_plot +
  ...
  labs(
    title = "Buffer of 100,000 (in metres)",
    subtitle = "With max_cells = 100 in st_buffer()"
  )

row_3_title <- "Without British Grid Projected CRS"
row_3_subtitle <- "Using London data in Projected CRS"
