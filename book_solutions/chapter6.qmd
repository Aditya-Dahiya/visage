...
-   **(1) Points from Raster Centroids**:
    -   Use `terra::as.points()` [@terra] for converting raster cells (non-NA) to points.
    -   Then, convert to `sf` objects with `sf::st_as_sf()`. [@sf-4]
...
-   **(2) Contour Lines (Isolines)**:
    ...
    -   Use `terra::as.contour()` or `rasterVis::contourplot()` for creating and overlaying contours. For example, visualize DEMs with hillshading and contours in @fig-6-5-2
...
grain_poly = as.polygons(grain, aggregate = FALSE) |> 
  st_as_sf()
grain_poly1 = as.polygons(grain, aggregate = TRUE) |> 
  st_as_sf()

grain_poly2 <- grain_poly1 |> 
