...
The object `above_3100` contains the 19 peaks which are above 3100 metres. Further, the `template_raster` is a template raster with a resolution of 3 km and the extent of the new point dataset. The results for count and maximum height in each cell os new raster are shown in @fig-e3.
...
The code below aggregates the raster showing the count of high points into a resolution of 6 km, using `terra::resample()` with a `template_raster_2` of resolution 6 km. The results are shown in @fig-e4.\
**Advantage****s of Reducing Raster Resolution**
...
The code chunks below show the polygonization, and filtered data of all squares representing clay (using `as.polygon(aggregate = FALSE)`). The @fig-e5 shows the `grain` dataset and the polygonized `grain_sf` where each row represents a \<POLYGON\> (aggregated).
...
![Figure showing the (a) raster data for \`grain\`, and (b) the polygonized data with `as.polygons(aggregate = FALSE)`](images/chapter6-e5.png){#fig-e5}
