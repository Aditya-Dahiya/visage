...
filters:
  - social-share
share:
  permalink: "https://aditya-dahiya.github.io/visage/book_solutions/chapter4.html"
...
```{r}
#| label: fig-e5-1
#| fig-cap: "Classifying elevation into high, middle and low"
#| fig-subcap: 
#|   - "Base raster"
#|   - "Reclassified raster"
...
```{r}
#| label: tbl-e5-2
#| tbl-cap: "Mean elevation and NDVI for each altitude class"
...
```{r}
#| label: fig-e6
#| fig-cap: "Edge detection with terra::focal()"
#| fig-subcap: 
#|   - "Base raster Logo"
#|   - "Horizontal edges detected with fx"
#|   - "Vertical edges detected with fy"
#| layout-ncol: 3
...
```{r}
#| label: tidy-code-with-pipe-operator
...
The `ggplot2` visualization in @fig-e7 uses facets to compare NDVI and NDWI values across the area, with clear labelling and a perceptually uniform colour scale.

```{r}
#| label: fig-e7
#| fig-cap: "Comparing NDVI and NDWI values"
...
This code demonstrates the steps to compute and visualize the distance to the nearest coastline for Spain. First, it loads elevation data for Spain and nearby areas ( @fig-e8-1 ), as well as for Spain's political boundaries ( @fig-e8-2 ), ensuring the inclusion of surrounding countries and water bodies. The raster is then aggregated for efficiency.
...
A binary raster is created where water is marked as `TRUE` (1) and land as `NA`. The `distance()` function calculates the distance of each land cell to the nearest water cell, with values converted to kilometres. ( @fig-e8-3 )

Finally, the raw distance raster is masked to display only distances within Spain, producing a final visualization of Spain with distances to its nearest coastline ( @fig-e8-4 ). The @fig-e8 includes multiple stages of the process, from raw data to the final processed map.

```{r}
#| label: fig-e8
#| fig-cap: "Steps in Computing the distance to nearest Coastline for Spain"
#| fig-subcap: 
#|   - "Land map of Spain and nearby areas"
#|   - "Land map of only Spain"
#|   - "Raw Spatraster of distance from coastline for entire landmass"
#|   - "Final product: Spain with distances to nearest Coastline"
...
```{r}
#| label: fig-e9
#| fig-cap: "Algebra on Rasters"
#| fig-subcap: 
#|  - "Elevation map of Spain"
#|  - "Spain: euclidean distances to nearest Coastline"
#|  - "Spain: distance to coastline weighted by elevation"
#|  - "Difference between 2nd and 3rd maps"
