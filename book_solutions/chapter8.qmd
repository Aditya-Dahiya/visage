...
file_access_table <- tibble(
  Prefix = c(
    "/vsicurl/", "/vsizip/", "/vsi7z/", "/vsis3/", "/vsigs/", "/vsicurl_streaming/"
  ),
  Description = c(
    "Access files over HTTP, HTTPS, or FTP.",
    "Read files directly from ZIP archives without extracting.",
    "Read files directly from 7z archives without extracting.",
    "Access files stored in Amazon S3 buckets.",
    "Access files stored in Google Cloud Storage.",
    "Stream files over HTTP, HTTPS, or FTP without seeking capabilities."
  ),
  `Example Usage` = c(
    'read_sf("/vsicurl/https://example.com/vector_file.shp")\nrast("/vsicurl/https://example.com/raster_file.tif")',
    'read_sf("/vsizip/path/to/archive.zip/contained_file.shp")\nrast("/vsizip/path/to/archive.zip/contained_file.tif")',
    'read_sf("/vsi7z/path/to/archive.7z/contained_file.shp")\nrast("/vsi7z/path/to/archive.7z/contained_file.tif")',
    'read_sf("/vsis3/bucket_name/path/to/file.shp")\nrast("/vsis3/bucket_name/path/to/file.tif")',
    'read_sf("/vsigs/bucket_name/path/to/file.shp")\nrast("/vsigs/bucket_name/path/to/file.tif")',
    'read_sf("/vsicurl_streaming/https://example.com/vector_file.geojson")\nrast("/vsicurl_streaming/https://example.com/raster_file.tif")'
  )
)
...
file_access_table |> 
  ...
  tab_header(
    ...
    subtitle = "Prefixes, Descriptions, and Example Usage for sf::read_sf() & terra::rast()"
  ) |> 
  tab_style(
    style = list(
      cell_text(
    ...
    locations = cells_body(
      columns = c(Prefix, `Example Usage`)
    )
  ) |> 
  cols_width(
    Prefix ~ px(150),
    Description ~ px(280),
    `Example Usage` ~ px(200)
  ) |> 
  gtExtras::gt_theme_538()
```
