...
-   **Purpose**:
    ...
    -   Identifies or extracts areas of overlap or subsets of spatial features.

    -   Commonly used in geographic data analysis to focus on regions of interest.

-   **Logical Operations and Spatial Equivalents**:

    -   Inspired by [Figure 12.1](https://r4ds.hadley.nz/logicals.html#fig-bool-ops) of [*R for Data Science (2e)*](https://r4ds.hadley.nz/).

    -   Spatial equivalents to logical operators (e.g., AND, OR, NOT) allow flexible geometry subsetting:
...
```{r}
sysfonts::font_add_google("Fira Code", "body_font")
showtext::showtext_auto()
...
g4 <- g1 +
  ...
  ggtitle("D |> st_difference(A) |> st_difference(B)")
...
g9 <- g1 +
  ...
  labs(title = "A st_union() and st_interaction() chain")
...
-   **Clipping**: Modifies geometry to match a subsetting object. **Subsetting**: Selects features that intersect or partly intersect with a clipping object.
-   **Illustration**: Points randomly distributed within the bounding box enclosing two overlapping circles (`x` and `y`). Some points are inside one circle, both circles, or neither. Then, we subset points intersecting with **both** `x` and `y`.
-   **Key Functions**:
    ...
    2.  **Clipping and Subsetting Approaches**:
        ...
        -   **Way #2**: Find the intersection between points (`p`) and `x_and_y`, modifying overlapping geometries: `st_intersection(p, x_and_y)`
...
-   **Preferred Implementation**:
    -   **Way #2** (concise and efficient) and it is the tidyverse approach with `|>` compatibility
    -   **Way #2 and #3** are alternatives with more control over geometry modifications.
