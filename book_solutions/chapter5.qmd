...
geometry_table |> 
  ...
  tab_style(
    style = cell_text(size = px(10)), # Set the desired font size in pixels
    locations = cells_body(
      columns = `Input Geometry Type`, # Apply to only one column
      rows = everything()              # Apply to all rows
    )
  ) |> 
  tab_style(
    style = cell_text(size = px(10)),
    locations = list(
      cells_column_labels(columns = everything()), # Apply to column labels
      cells_stub(rows = everything())             # Apply to row labels (stub)
    )
  )
