...
Table 5.1: Geometry Type Transformations for Simple Features *(Credits: [Geomputation with R](https://r.geocompx.org/), Book [Table 5.1](https://r.geocompx.org/geometry-operations#tab:sfs-st-cast))*

| Input Geometry Type | POINT | MULTIPOINT | LINESTRING | MULTILINESTRING | POLYGON | MULTIPOLYGON | GEOMETRYCOLLECTION |
|---------|---------|---------|---------|---------|---------|---------|---------|
| POINT (1 point) | 1 | 1 | 1 | NA | NA | NA | NA |
| MULTIPOINT (4 points) | 4 | 1 | 1 | 1 | NA | NA | NA |
| LINESTRING (1 linestring with 5 points) | 5 | 1 | 1 | 1 | NA | NA | NA |
| MULTILINESTRING (2 linestrings: one with 5 points, one with 2 points) | 7 | 2 | 2 | 1 | NA | NA | NA |
| POLYGON (1 polygon with 5 points) | 5 | 1 | 1 | 1 | 1 | NA | NA |
| MULTIPOLYGON (2 polygons: each with 5 points) | 10 | 1 | NA | 1 | 2 | 1 | 1 |
| GEOMETRYCOLLECTION (2 geometries: a MULTIPOINT with 4 points and a LINESTRING with 5 points) | 9 | 1 | NA | NA | NA | NA | 1 |

An attempt to recreate this table graphically with {ggplot2} and {sf}:
...
ggplot(temp) +
  ...
  labs(
    title = if_else(
      i == 1,
      str_remove(class(temp$geometry), "sfc_")[1],
      ""
    )
  ) +
...
![Comparising base R `[]` and `terra::crop()` for subsetting rasters](images/chapter5-3-1_1.png){#fig-5-3-1}
