...
-   **Efficient alternatives for operations**:
    -   **`app()`**: Applies a function to each cell across raster layers.
    -   **`tapp()`**: Extends `app()` by allowing layer subsets for operations.
    -   **`lapp()`**: Applies functions using multiple layers as arguments. An example of `lapp()` is the NDVI Calculation:
...
-   **Predictive mapping**:
    -   Combines local raster operations with predictive modeling.
    -   Predictive variables (e.g., elevation, temperature) are extracted from rasters to model responses like species richness or crop yield.
    -   Models can include **`lm()`**, **`glm()`**, **`gam()`**, or machine-learning techniques.
