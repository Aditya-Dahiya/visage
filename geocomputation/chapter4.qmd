...
```{r}
#| label: fig-4-3
#| fig-cap: "Section 4.3: Raster Datasets used"
#| fig-subcap: 
#|    - "elev"
#|    - "grain"
#| layout-ncol: 2
...
-   Spatial subsetting allows extraction of raster data by **location (coordinates)** or **spatial objects**:
    -   **`terra::extract()`**: Extracts raster values directly using coordinates. (Note: A function with the same name exists in the **tidyverse**).
-   **Subsetting with another raster object**:
    -   Use a secondary raster as a spatial mask to subset the primary raster.
    -   Example: Subsetting `elev` using a smaller raster `clip_raster` (defining a specific extent).
-   **Spatial outputs from subsetting**:
...
-   The `extract()` function in the **terra** package retrieves values from a `SpatRaster` based on specified locations or geometries. Locations can be points (as a `SpatVector`, matrix, or data frame), cell numbers, or spatial objects like polygons. It supports methods for exact or weighted extraction, interpolation, and summary statistics for extracted data. Key arguments include:
    ...
    -   **`method`**: Extraction type for points ("simple" or "bilinear").
...
```{r}
#| label: fig-4-3-1
#| fig-cap: "Showing plots from the code of above Section 4.3.1"
#| fig-subcap: 
#|   - "clip_raster: a smaller raster"
