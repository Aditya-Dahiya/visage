...
bbox_orcas <- bind_rows(
  ...
  # Geometry column of starting coordiantes
    sf::st_as_sf(
      df1 |> select(begin_latitude, begin_longitude),
  ...
  # Geometry column of starting coordiantes
    sf::st_as_sf(
      df1 |> select(end_latitude, end_longitude),
...
routes <- geosphere::gcIntermediate(
  p1 = df_sf |> select(begin_longitude, begin_latitude),
  p2 = df_sf |> select(end_longitude, end_latitude),
  n = 100,
  addStartEnd = TRUE,
  sp = TRUE) |> 
  sf::st_as_sf()
